...
def generate_combinated_file():
    ...
    for i in range(1, 16):
        user = "user" + str(i)
        for folder in os.listdir("data/users/stage-2/" + user + "/old"):
            for file in os.listdir("data/users/stage-2/" + user + "/old/" + folder):
                predicted_df = pd.read_csv("data/users/stage-2/" + user + "/old/" + folder + "/" + file).dropna()[
                    columns]
...
def generate_combinated_file_new_actions():
    ...
    for i in range(1, 16):
        if i != 10:
            user = "user" + str(i)
            for folder in os.listdir("data/users/stage-2/" + user + "/new"):
                for file in os.listdir("data/users/stage-2/" + user + "/new/" + folder):
                    predicted_df = pd.read_csv("data/users/stage-2/" + user + "/new/" + folder + "/" + file).dropna()[
                        columns]
                    real_df = copy.deepcopy(predicted_df)
                    real_df["action"] = folder
                    real_data_df_list.append(real_df)
